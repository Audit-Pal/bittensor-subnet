...
Welcome to AuditPal! This guide will walk you through setting up your development environment, whether you're on Windows, Linux, or macOS. Follow these steps carefully to get your miner or validator running smoothly.
...
If you're on Windows, you'll need Windows Subsystem for Linux (WSL2) to run Bittensor.
...
Register your hotkey on the AuditPal subnet:

```bash
btcli subnet register --netuid  --subtensor.network test --wallet.name miner --wallet.hotkey default
```
...
```bash
python -m neurons.miner --netuid 310
...
**Ready to audit smart contracts? Let's go! 🚀**